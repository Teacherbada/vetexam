...
export default function SubjectsPage() {
  ...
  return <main className={`${foundation.foundation} ${layout.page}`}><div className={layout.container}>
    <nav className={layout.breadcrumb} aria-label="麵包屑"><Link href="/" aria-label="回首頁"><StudyIcon name="home" />首頁</Link><span aria-hidden="true">/</span><span aria-current="page">選擇科目</span><Link href="/questions/search">題目搜尋</Link></nav>
    <PageHeader title="選擇科目" description="選擇你想練習的科目，開始今天的刷題。"><Link href="/most-missed" className="study-button"><StudyIcon name="target" />最多人答錯</Link></PageHeader>
  ...
  {selectedSubject && <dialog className={styles.dialog} aria-labelledby="subject-settings-title" ref={node => { if (node && !node.open) node.showModal(); }} onClose={() => setSelectedSubject(null)}>
    <div>
      <div className="flex items-start justify-between"><div><h2 id="subject-settings-title" className="text-2xl font-bold">開始刷題</h2><p className="mt-1 text-gray-500">每組條件獨立出題，題數不足時使用全部符合題目；重複題目只計一次。</p></div><button aria-label="關閉練習設定" onClick={event => event.currentTarget.closest("dialog")?.close()} className="min-h-11 min-w-11 rounded-full px-3 py-2 text-[#6F7873] hover:bg-[#E9F2ED]">✕</button></div>
      ...
      <section className="mt-7"><h3 className="font-bold">題目順序</h3><div className="mt-3 grid grid-cols-2 gap-3"><button aria-pressed={order === "original"} onClick={() => setOrder("original")} className={`rounded-2xl border p-4 text-left transition ${order === "original" ? "border-[#5F8F7B] bg-[#E9F2ED] ring-1 ring-[#E9F2ED]" : "border-gray-200 hover:bg-gray-50"}`}><b>原始順序</b><div className="mt-1 text-sm text-gray-500">依年份、原題題號排列</div></button><button aria-pressed={order === "random"} onClick={() => setOrder("random")} className={`rounded-2xl border p-4 text-left transition ${order === "random" ? "border-[#5F8F7B] bg-[#E9F2ED] ring-1 ring-[#E9F2ED]" : "border-gray-200 hover:bg-gray-50"}`}><b>隨機順序</b><div className="mt-1 text-sm text-gray-500">每次測驗重新打亂</div></button></div></section>
