...
import { TableRegion } from "@/components/ui/AdminShell";
...
export default function SystemHealth() {
  ...
  return <section className="space-y-4 rounded-xl bg-white p-5" aria-labelledby="system-health-title">
    ...
    {error ? <p role="alert" className="text-amber-800">{error}</p> : !data ? <LoadingState label="讀取錯誤紀錄中…" /> : <>
      ...
      {data.errors.length === 0 ? <EmptyState title="近 7 天尚無已記錄的錯誤。" /> : <TableRegion label="錯誤紀錄"><table className="w-full text-left text-sm"><thead><tr><th scope="col" className="p-2">時間</th><th scope="col" className="p-2">操作</th><th scope="col" className="p-2">分類</th></tr></thead><tbody>{data.errors.map(entry => {
