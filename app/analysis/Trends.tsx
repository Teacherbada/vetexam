...
export default function Trends({ history }: { history: History[] }) {
  ...
  return <section className={styles.subjectSection} aria-labelledby="trends-title">
    ...
    <div className={styles.subjectGrid}>{([['最近 7 天',trend.recent],['前 7 天',trend.previous],['最近 30 天',trend.month],['累積練習',trend.all]] as const).map(([label, row]) => <article className={styles.subjectCard} key={label}><h3>{label}</h3><strong>{percent(row.accuracy)}</strong><p>{row.count} 題</p></article>)}</div>
    <p className={styles.notice}>最近 7 天相較前 7 天：{trend.change === null ? '近期資料不足' : `${trend.change >= 0 ? '+' : ''}${Math.round(trend.change)} 個百分點`}</p>
    <div className={styles.subjectGrid}>{trend.subjects.map(row => <article className={styles.subjectCard} key={row.subject}><h3>{row.subject}</h3><p>{row.change === null ? '近期資料不足' : `${percent(row.previous.accuracy)} → ${percent(row.recent.accuracy)}`}</p><small>前期 {row.previous.count} 題 · 最近 {row.recent.count} 題</small></article>)}</div>
