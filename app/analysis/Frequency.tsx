...
export default function Frequency({ history }: { history?: History[] }) {
  ...
  return <section className={styles.subjectSection} aria-labelledby="frequency-title">
    ...
    <div className={styles.toolbar}><label>科目 <select className="max-w-full rounded-xl border p-3" value={subject} onChange={event => setSubject(event.target.value)}>{EXAM_SUBJECTS.map(name => <option key={name}>{name}</option>)}</select></label><div className={styles.filters} role="group" aria-label="出題期間">{([null,10,5] as const).map(value => <button key={value ?? 'all'} aria-pressed={years === value} className={`study-button ${years === value ? 'study-button-primary' : ''}`} onClick={() => setYears(value)}>{value ? `最近 ${value} 年` : '全期間'}</button>)}</div></div>
