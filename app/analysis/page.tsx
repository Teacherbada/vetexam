...
export default function AnalysisPage() {
  ...
  const account = useSyncExternalStore(subscribeLearning, getLearning, getServerSnapshot);
  const status = useSyncExternalStore(subscribeLearning, getLearningStatus, () => 'loading');
  const signedIn = Boolean(learningOwner());
  const analysis = buildAnalysis(signedIn ? JSON.stringify(account?.progress ?? {}) : raw ?? "{}");
  ...
  return <main className={`${foundation.foundation} ${styles.page}`}>
    <div className={styles.container}>
      ...
      {loading ? <section className={styles.empty} aria-busy="true" role="status">正在整理你的學習紀錄…</section> : <>
        ...
        <Frequency history={signedIn ? account?.history : undefined} />
        ...
        <aside className={styles.method}><StudyIcon name="book" /><div><h2>關於這份分析</h2><p>掌握度以「答對題數 ÷ 已作答題數」計算，僅供練習參考。依現有紀錄規則，同一科目的同一題只計入首次作答；重做不會更新這份累積正確率。</p><p>{signedIn ? '上方統計來自帳號首次作答紀錄，手機與電腦使用相同資料。舊裝置摘要另外保留，避免重複計算。' : '未登入時顯示此瀏覽器的紀錄；登入後可使用帳號學習紀錄。'}</p></div></aside>
      </>}
    </div>
  </main>;
}
