...
import { parseSearch, SEARCH_SUBJECTS, searchUrl, type SearchParams } from '@/lib/question-search';
  ...
  return <main className={styles.page}><div className={styles.container}>
    ...
    <section className={styles.card}><h1 className={searchStyles.title}>題目搜尋</h1><p>搜尋歷屆國考題，找到想釐清的觀念。</p>
      <form action="/questions/search" className={searchStyles.form} key={searchUrl(filters)}>
        ...
        <label>科目<select aria-label="科目" name="subject" defaultValue={filters.subject}><option value="">全部科目</option>{SEARCH_SUBJECTS.map(subject => <option key={subject}>{subject}</option>)}</select></label>
    ...
    <section aria-label="搜尋結果" className={searchStyles.results}>
      {filters.invalid ? <p role="status">請確認科目、年份與題號格式。</p> : !filters.active ? <p>輸入關鍵字，或選擇科目、年份開始搜尋。</p> : !result.rows.length ? <p role="status">找不到符合條件的題目。請移除年份或改用較短關鍵字。</p> : result.rows.map(row => <article className={styles.card} key={row.id}>
