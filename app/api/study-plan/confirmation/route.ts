...
import { answerConfirmation, readConfirmation, startConfirmation } from "@/lib/confirmation-service";
...
async function handle(request: Request, action: "read" | "start" | "answer") {
  try {
    ...
    if (!session?.user?.id) return json({ error: "請先登入後使用弱點確認。" }, 401);
    const userId = session.user.id;
    if (action === "read") return json(await questionTransaction(client => readConfirmation(client, userId)));
    if (action === "start") return json(await questionTransaction(client => startConfirmation(client, userId)));
    ...
    return json(await questionTransaction(client => answerConfirmation(client, userId, submission)));
  } catch (error) {
    if (error instanceof DiagnosticError) return json({ error: error.message }, error.status);
    return json({ error: "弱點確認暫時無法連線。請重新載入確認已儲存進度。" }, 503);
  }
}
