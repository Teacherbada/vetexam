...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const questionSets = await sql`
      SELECT id, name, filename, total_questions, created_at, file_hash, visibility, owner_id
      FROM question_sets
    ...
    const questions = await sql`
      SELECT id, question_set_id, question_number, subject, question,
             option_a, option_b, option_c, option_d, answer, explanation,
             image_data_url, created_at
      FROM questions
    ...
    return NextResponse.json({ error: "無法取得題目", detail: error instanceof Error ? error.message : "未知錯誤" }, { status: 500 });
  }
}
