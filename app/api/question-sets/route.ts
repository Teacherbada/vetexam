...
export async function GET(request: Request) {
  try {
    ...
    const questionSets=userId?await sql`SELECT id,name,filename,total_questions,created_at,file_hash,visibility,owner_id,exam_subject,exam_year FROM question_sets WHERE visibility='public' OR (visibility='private' AND owner_id=${userId}) ORDER BY created_at DESC`:await sql`SELECT id,name,filename,total_questions,created_at,file_hash,visibility,owner_id,exam_subject,exam_year FROM question_sets WHERE visibility='public' ORDER BY created_at DESC`;
    return NextResponse.json({success:true,questionSets});
  }catch(error){console.error("Question sets API error:",error);return NextResponse.json({error:"取得題庫失敗",detail:error instanceof Error?error.message:"未知錯誤"},{status:500});}
}
...
export async function POST(request: Request) {
  try {
    ...
    // Backward-compatible schema upgrade for the optional fifth choice.
    ...
    if(fileHash){
      const existing=isAdmin?await sql`SELECT id,name,owner_id,visibility FROM question_sets WHERE file_hash=${fileHash} AND visibility=${visibility} LIMIT 1`:await sql`SELECT id,name,owner_id,visibility FROM question_sets WHERE file_hash=${fileHash} AND (visibility='public' OR (visibility='private' AND owner_id=${userId})) LIMIT 1`;
      if(existing.length){const existingSet=existing[0];const message=existingSet.visibility==='public'?"這份 PDF 已經存在於公開題庫，不需要再次匯入。":"這份 PDF 已經存在於相同的題庫範圍，不需要再次匯入。";return NextResponse.json({error:message,code:"DUPLICATE_FILE",questionSetId:Number(existingSet.id),visibility:existingSet.visibility},{status:409});}
    }
...
export async function DELETE(request: Request) {
  ...
  }catch(error){console.error("Delete question set error:",error);return NextResponse.json({error:"刪除題庫失敗",detail:error instanceof Error?error.message:"未知錯誤"},{status:500});}
}
