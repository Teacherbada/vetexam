...
import { DIAGNOSTIC_CONFIG, selectDiagnosticQuestions, summarizeDiagnostic, type Candidate, type DiagnosticView, type parseDiagnosticAnswer } from "@/lib/diagnostic";
...
async function planMode(client: PoolClient, userId: string, write: boolean) {
...
export async function readDiagnostic(client: PoolClient, userId: string): Promise<DiagnosticView> {
  const mode = await planMode(client, userId, false);
  const { rows: sessions } = await client.query("SELECT id, completed_at FROM diagnostic_sessions WHERE user_id = $1", [userId]);
  ...
  return { mode, session: {
    ...
    shortages: results.filter(row => row.total < DIAGNOSTIC_CONFIG.questionsPerSubject).map(row => ({ subject: row.subject, available: row.total })),
...
export async function startDiagnostic(client: PoolClient, userId: string): Promise<DiagnosticView> {
  ...
  const existing = await client.query("SELECT id FROM diagnostic_sessions WHERE user_id = $1", [userId]);
  ...
  const id = randomUUID();
  await client.query("INSERT INTO diagnostic_sessions (id, user_id) VALUES ($1, $2)", [id, userId]);
...
export async function answerDiagnostic(client: PoolClient, userId: string, submission: NonNullable<ReturnType<typeof parseDiagnosticAnswer>>): Promise<DiagnosticView> {
  await planMode(client, userId, true);
  const { rows: sessions } = await client.query("SELECT id FROM diagnostic_sessions WHERE user_id = $1 AND id = $2 FOR UPDATE", [userId, submission.sessionId]);
  ...
  if (item.selected_answer !== null) {
    ...
    return readDiagnostic(client, userId); // Safe retry after an ambiguous network failure.
  }
