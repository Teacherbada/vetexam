...
export type ImportQuestion = { questionNumber?: number; question: string; options: string[]; answer: string; explanation: string; imageDataUrl?: string | null; chapter?: string | null };
  ...
  return ranges.map((range, batchIndex) => {
    const batch: ImportBatch = { ...metadata, importId, ranges, totalQuestions: questions.length, batchIndex,
      questions: questions.slice(range.from - 1, range.to).map((question) => ({
        ...(question.questionNumber !== undefined ? { questionNumber: question.questionNumber } : {}),
