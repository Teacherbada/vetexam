...
import legacyStyles from '@/app/questions/quiz.module.css';
...
export default function QuestionCard({ currentQuestion, selected, showResult, answered, isFavorite, favoriteQuestion, chooseAnswer, focused = false, appearance = legacyStyles, image }: {
  focused?: boolean; appearance?: typeof legacyStyles; image?: ReactNode; currentQuestion: QuizQuestion; selected: string; showResult: boolean; answered: boolean;
  ...
  return <>
      {!focused && <div className={styles.questionHeader}><span>單選題</span><button onClick={favoriteQuestion} aria-pressed={isFavorite} className={styles.favorite}><StudyIcon name="heart" />{isFavorite ? "已收藏" : "收藏題目"}</button></div>}
      <h1 className={styles.question} key={currentQuestion.id}>{currentQuestion.question}</h1>
      {currentQuestion.imageDataUrl && (image ?? <img key={`image-${currentQuestion.id}`} src={currentQuestion.imageDataUrl} alt={`第 ${currentQuestion.questionNumber} 題圖片`} className={styles.questionImage} />)}
      ...
      {focused && <div className={styles.questionHeader}><button onClick={favoriteQuestion} aria-pressed={isFavorite} className={styles.favorite}><StudyIcon name="heart" />{isFavorite ? "已收藏" : "收藏題目"}</button></div>}
  </>;
}
