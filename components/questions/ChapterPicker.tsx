...
export default function ChapterPicker({ subject, value, onChange, name, emptyLabel = '全部章節', allowEmpty = true }: {
  subject: string; value: string; onChange: (chapter: string) => void; name?: string;
  emptyLabel?: string; allowEmpty?: boolean;
}) {
  const groups = chapterGroups(subject);
  return <div className={styles.root}>
    {name && <input type="hidden" name={name} value={value} />}
    <details className={styles.picker}>
      <summary>章節：{value || emptyLabel}</summary>
      <div className={styles.options} role="group" aria-label={`${subject || '全部科目'}章節`}>
        {allowEmpty && <button type="button" aria-pressed={!value} onClick={() => onChange('')} className={`study-button ${styles.choice} ${!value ? 'study-button-primary' : ''}`}>{emptyLabel}</button>}
