...
async function setup({source='weekly',answer='E',fail=false}={}){
 ...
 await page.goto(base);await page.locator('.study-register').waitFor();return {page,posts,attempts:()=>attempts};
}
try{
 const {page,posts,attempts}=await setup();
 const toggle=page.getByRole('button',{name:'挑戰這題'});await toggle.waitFor();assert.equal(await toggle.getAttribute('aria-expanded'),'false');assert.equal(attempts(),0);
 for(const width of [320,375,430,768,1024,1280,1440]){await page.setViewportSize({width,height:1000});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));if([375,1280].includes(width))await page.locator('.study-weekly').screenshot({path:out+'/collapsed-'+width+'.png'});}
 await toggle.click();const group=page.getByRole('group',{name:'本週最多人答錯答案選項'});await group.waitFor();assert.equal(await group.getByRole('button').count(),5);assert.equal(await page.getByRole('dialog').count(),0);assert.equal(posts.length,0);
 ...
 await page.getByRole('button',{name:'收合題目'}).click();assert.equal(await group.isVisible(),false);await page.getByRole('button',{name:'挑戰這題'}).click();assert.equal(await group.locator('button:disabled').count(),5);assert.equal(attempts(),1);assert.equal(posts.filter(p=>p.path==='/api/stats/answers').length,1);
